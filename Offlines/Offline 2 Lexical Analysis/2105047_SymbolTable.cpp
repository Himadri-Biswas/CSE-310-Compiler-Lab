...
class ScopeTable
{
    ...
    ScopeTable(int uniqId, int totalBuckets, ScopeTable* parentScope, int hashFunctionNum=1)
    {
        ...
        if (parentScope == nullptr) {
            nestedId = to_string(uniqId); // Root scope (e.g., "1")
        } else {
            nestedId = parentScope->getNestedId() + "." + to_string(uniqId); // Nested scope (e.g., "1.1")
        }
    ...
    unsigned int sdbm_hash(const std::string& str) {
        return sdbm_hash(str.c_str());
    }
    
    // unsigned long long int sdbm_hash(const string &str)
    // {
    ...
    void setNestedId(string nestedId)
    {
    ...
    bool insert(SymbolInfo& symbol, ofstream& outfile)
    {
        if(lookUp(symbol.getName(),outfile,0)!=NULL) // we won't print the lookup meessage in this case, so check=0
        {
            ...
            //outfile<<"\t"<<"'"<<symbol.getName()<<"' already exists in the current ScopeTable"<<endl<<endl;
            // say the position also when printing the message
            //outfile<<"'"<<symbol.getName()<<"' already exists in the current ScopeTable at position "<<bucketNum+1<<", "<<position<<endl;
            outfile << "< " << symbol.getName() << " : " << symbol.getType() << " > already exists in ScopeTable# " << nestedId << " at position " << bucketNum << ", " << position -1 << endl<<endl;
            return false;
        }
    ...
    // same print funtion but print only non-empty buckets , and no indentation needed
    void print2(ofstream& outfile)
    {
        //cout << "\t" << "ScopeTable# " << uniqId << endl;
        //outfile << "ScopeTable # " << uniqId << endl;
        outfile << "ScopeTable # " << nestedId << endl;

        // i is the index of the bucket in the hash table(actual index; starts from 0)
        for(int i = 0; i < totalBuckets; i++)
        {
            if(symbolList[i] == NULL) // if the bucket is empty, skip it
            {
                continue;
            }
            // j is also the index of the bucket in the hash table but it starts from 1 for printing purpose
            ...
            //     outfile<<endl;
            // }
            outfile << endl; // Print a new line after printing the bucket
        }
        //outfile << endl; // Print a new line after printing the scope table 
    }
...
class SymbolTable
{
    ...
    int childCounter = 1; // Add this to track child scope counts
    ...
    SymbolTable(int uniqId, int totalBuckets,ofstream& outfile)
    {
        currentScope = NULL;

        ScopeTable* newScope = new ScopeTable(uniqId, totalBuckets, currentScope); // Creating the first scope table with the given unique id and total number of buckets
        currentScope = newScope; // Setting the new scope table as the current scope table
    }
    ...
    //     // For nested IDs, use `childCounter` instead of `uniqId`
    //     ScopeTable* newScope = new ScopeTable(childCounter, totalBuckets, currentScope, hashFunctionNum);
    //     currentScope = newScope;
    //     childCounter++; // Increment for next child under the same parent
    ...
    void enterScope(int uniqId, int totalBuckets, ofstream& outfile, int hashFunctionNum) {
        // Reset childCounter to 1 for new nesting level
        childCounter = 1;
    ...
    void printCurrentScopeTable(ofstream& outfile)
    {
        ...
        currentScope->print2(outfile); // Printing the current scope table in the specified format
        return;
    }
    ...
    void printAllScopeTable(ofstream& outfile)
    {
        ...
        while (temp != NULL)
        {
            ...
            temp->print2(outfile); // Printing the current scope table in the specified format
            temp = temp->getParentScope();
            indent++;
        }

        outfile << endl; // Print a new line after printing all scope tables
    }
